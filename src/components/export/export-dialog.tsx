...
export default function ExportDialog({ open, onOpenChange, uid, manuscript, chapters }: Props) {
    ...
    loadBookMeta(uid, manuscript.id)
      .then((m) => {
        ...
        if (m) {
          ...
          if (m.settings) setSettings({ ...DEFAULT_EXPORT_SETTINGS, ...m.settings, page: { ...DEFAULT_EXPORT_SETTINGS.page, ...m.settings.page } });
        }
      })
      .catch(() => {})
  ...
  const importCover = async (file: File) => {
    ...
    try {
      const data = await compressImage(await fileToDataUrl(file), { maxWidth: 1200, maxHeight: 1800, maxBytes: MAX_COVER_BYTES, cover: true });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="w-[min(96vw,1040px)] max-h-[min(92dvh,900px)]"
      >
        ...
        {format === 'md' ? (
          <div className="rounded-xl border border-border p-6 text-center">
            <p className="text-sm text-muted">Texte brut balisé, idéal pour un correcteur, Pandoc ou un dépôt Git. Notes de bas de page incluses.</p>
          ...
          <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_280px]">
              ...
              <div className="mb-4 flex gap-1 border-b border-border">
                {(
                  [
                    ['book', 'Le livre'],
                    ['cover', 'Couverture'],
                    ...(format === 'pdf' ? [['layout', 'Mise en page']] : []),
                  ] as [Section, string][]
                ).map(([id, label]) => (
                  <button
                    key={id}
              ...
              {section === 'book' && (
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Auteur·rice (nom de plume)">
                    <Input value={meta.penName || meta.authorName} onChange={(e) => setMeta({ ...meta, penName: e.target.value, authorName: e.target.value })} maxLength={120} />
                  </Field>
                  <Field label="Éditeur">
                    <Input value={meta.publisher ?? ''} onChange={(e) => setMeta({ ...meta, publisher: e.target.value })} placeholder="Auto-édition" maxLength={120} />
                  </Field>
                  <Field label="ISBN">
                    <Input value={meta.isbn ?? ''} onChange={(e) => setMeta({ ...meta, isbn: e.target.value.replace(/[^0-9Xx-]/g, '') })} placeholder="978-…" maxLength={20} />
                  </Field>
                  <Field label="Année">
                    <Input type="number" value={meta.copyrightYear ?? ''} onChange={(e) => setMeta({ ...meta, copyrightYear: Number(e.target.value) || undefined })} />
                  </Field>
                  <Field label="Dédicace" className="sm:col-span-2">
                    <Textarea value={meta.dedication ?? ''} onChange={(e) => setMeta({ ...meta, dedication: e.target.value })} rows={2} className="min-h-0" maxLength={1000} />
                  </Field>
                  <Field label="Épigraphe" className="sm:col-span-2">
                    <Textarea value={meta.epigraph ?? ''} onChange={(e) => setMeta({ ...meta, epigraph: e.target.value })} rows={2} className="min-h-0" maxLength={1000} />
                  </Field>
                  <Field label="Quatrième de couverture" className="sm:col-span-2">
                    <Textarea value={meta.backCoverBlurb ?? ''} onChange={(e) => setMeta({ ...meta, backCoverBlurb: e.target.value })} rows={4} maxLength={2500} />
                  </Field>
                  {format === 'pdf' && (
                    <div className="sm:col-span-2">
                      <div className="mb-2 flex items-center justify-between">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          onClick={() =>
                            setSections([...sections, { id: createId('sec'), placement: 'front', kind: 'preface', title: 'Préface', content: '', order: sections.length }])
                          }
                        >
                        ...
                        {sections.map((s, i) => (
                          <div key={s.id} className="rounded-xl border border-border p-3">
                            <div className="flex gap-2">
                              <Input value={s.title} onChange={(e) => setSections(sections.map((x, j) => (j === i ? { ...x, title: e.target.value } : x)))} className="h-9" aria-label="Titre de la section" />
                              <select
                                value={s.placement}
                                onChange={(e) => setSections(sections.map((x, j) => (j === i ? { ...x, placement: e.target.value as 'front' | 'back' } : x)))}
                              ...
                              <Button size="icon-sm" variant="ghost" onClick={() => setSections(sections.filter((_, j) => j !== i))} aria-label="Supprimer la section">
                ...
                <div className="space-y-5">
                  <div className="grid grid-cols-4 gap-2">
                    {(
                      ...
                      const activeMode = id === 'ai' ? cover.mode === 'generated' && !!cover.imageUrl : id === 'generated' ? cover.mode === 'generated' && !cover.imageUrl : cover.mode === id;
                      return (
                        <button
                          key={id}
                          type="button"
                          ...
                          className={cn('h-10 rounded-lg border border-border text-[13px] font-medium text-muted transition hover:border-border-strong', activeMode && 'border-ember bg-ember-soft text-ember')}
                        >
                          {label}
                        </button>
                      );
                    })}
                    <input ref={fileRef} type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(e) => e.target.files?.[0] && void importCover(e.target.files[0])} />
                  </div>
                    ...
                    <div className="grid grid-cols-4 gap-2 sm:grid-cols-8">
                      {COVER_PALETTES.map((p) => (
                        <button
                          ...
                          onClick={() => setCover((c) => ({ ...c, mode: 'generated', imageUrl: undefined, background: { type: p.value.startsWith('linear') ? 'gradient' : 'color', value: p.value }, titleColor: p.text }))}
                          className={cn('aspect-[2/3] rounded-md ring-offset-2 ring-offset-surface transition hover:scale-105', cover.background?.value === p.value && !cover.imageUrl && 'ring-2 ring-ember')}
                  ...
                  <div className="rounded-xl border border-iris/30 bg-iris-soft/40 p-4">
                    ...
                    <Textarea value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} rows={2} className="min-h-0 bg-surface" placeholder="Ex. : un phare dans la brume, une silhouette sur le quai, lumière d’aube" maxLength={600} />
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      {(['editorial', 'illustration', 'photo', 'minimal', 'vintage'] as const).map((s) => (
                        <button key={s} type="button" onClick={() => setAiStyle(s)} className={cn('h-7 rounded-full border border-border bg-surface px-3 text-xs capitalize text-muted', aiStyle === s && 'border-iris text-iris')}>
                          {{ editorial: 'éditorial', illustration: 'illustration', photo: 'photo', minimal: 'minimal', vintage: 'vintage' }[s]}
                        </button>
                      ))}
                  ...
                  <div className="flex flex-wrap items-center gap-6">
                    <label className="flex items-center gap-3 text-sm">
                      <input type="color" value={cover.titleColor ?? '#ffffff'} onChange={(e) => setCover((c) => ({ ...c, titleColor: e.target.value }))} className="size-8 cursor-pointer rounded border border-border bg-transparent" />
                      Couleur du titre
                    </label>
                    <label className="flex items-center gap-3 text-sm">
                ...
                <div className="space-y-6">
                      ...
                      {(Object.keys(THEME_REGISTRY) as ThemeId[]).map((id) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn('rounded-xl border border-border p-2.5 text-left transition hover:border-border-strong', settings.themeId === id && 'border-ember ring-4 ring-ember/10')}
                          >
                            <span className={cn('block text-lg leading-none', t.fonts.heading.startsWith('serif') ? 'font-serif' : t.fonts.heading.startsWith('mono') ? 'font-mono' : 'font-sans')} style={{ color: t.colors.accent }}>
                              Aa {t.ornamentGlyph ?? ''}
                            </span>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                      {FORMATS.map((f) => (
                        <button key={f.id} type="button" onClick={() => setPage({ format: f.id })} className={cn('rounded-xl border border-border p-2.5 text-left', settings.page.format === f.id && 'border-ember bg-ember-soft/50')}>
                  ...
                  <div className="grid gap-5 sm:grid-cols-2">
                    <RangeField label="Corps du texte" value={settings.page.fontSizePt} unit="pt" min={9} max={14} step={0.5} onChange={(v) => setPage({ fontSizePt: v })} />
                    <RangeField label="Interlignage" value={settings.page.lineHeight} min={1.2} max={1.9} step={0.05} onChange={(v) => setPage({ lineHeight: Math.round(v * 100) / 100 })} />
                    <RangeField label="Marge intérieure" value={settings.page.marginInsideMm} unit="mm" min={10} max={35} step={1} onChange={(v) => setPage({ marginInsideMm: v })} />
                    <RangeField label="Marge extérieure" value={settings.page.marginOutsideMm} unit="mm" min={10} max={30} step={1} onChange={(v) => setPage({ marginOutsideMm: v })} />
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    {(
                      [
                        ...
                        ['Numéros de chapitre', settings.includeChapterNumbers, (v: boolean) => setSettings((s) => ({ ...s, includeChapterNumbers: v }))],
                      ] as const
            ...
            <aside className="space-y-4">
              <div className="mx-auto w-[200px] [perspective:900px]">
                {cover.mode === 'none' ? (
                  <div className="grid aspect-[2/3] place-items-center rounded-lg border border-dashed border-border text-xs text-faint">Sans couverture</div>
                ) : (
                  <div className="transition-transform duration-500 [transform:rotateY(-14deg)] hover:[transform:rotateY(0)]">
                    <BookCover
                      title={meta.title}
                      subtitle={meta.subtitle}
                      author={meta.penName || meta.authorName}
              ...
              {format === 'pdf' && (
                <div className="rounded-xl border border-border bg-paper p-4 shadow-soft">
                  <p className="text-center text-[10px] uppercase tracking-[0.2em]" style={{ color: theme.colors.accent }}>
                    Chapitre 1
                  </p>
                  <p className={cn('mt-1 text-center text-[15px]', theme.fonts.heading.startsWith('sans') ? 'font-sans font-semibold' : 'font-serif')}>{chapters[0]?.title}</p>
                  <p
                    className={cn('mt-3 line-clamp-6 text-[10.5px] leading-[1.55] text-text/80', settings.page.justify && 'text-justify', theme.fonts.body.startsWith('sans') ? 'font-sans' : 'font-serif')}
                    style={{ textIndent: settings.page.firstLineIndentMm ? '1.2em' : 0 }}
                  >
                    {chapters[0] ? chapterToMarkdown('', chapters[0].doc, []).replace(/^#\s*\n+/, '').replace(/[*_\\#>]/g, '').slice(0, 520) : ''}
                  </p>
                </div>
              )}
              <Button className="w-full" size="lg" onClick={() => void (format === 'pdf' ? makePdf() : makeEpub())} loading={busy === 'pdf' || busy === 'epub'} disabled={!loaded || !!busy}>
                {busy === 'pdf' || busy === 'epub' ? null : <Download className="size-4" />}
                {format === 'pdf' ? 'Générer le PDF' : 'Générer l’EPUB'}
              </Button>
              {pdfUrl && format === 'pdf' && (
                <a href={pdfUrl} target="_blank" rel="noopener" className="block text-center text-xs text-muted underline underline-offset-4">
...
function RangeField({ label, value, unit = '', min, max, step, onChange }: { label: string; value: number; unit?: string; min: number; max: number; step: number; onChange: (v: number) => void }) {
  return (
    <div>
