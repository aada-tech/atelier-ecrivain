 ...
 */
export function BookCover({ title, subtitle, author, background, imageUrl, textColor = '#fff', hideText, className, volume }: BookCoverProps) {
  return (
    <div
      className={cn(
        'relative aspect-[2/3] overflow-hidden rounded-[3px_8px_8px_3px] [container-type:inline-size]',
    ...
    >
      ...
      {!hideText && (
        <div className="relative flex h-full flex-col items-center justify-between px-[9%] pb-[10%] pt-[18%] text-center">
          <div>
          ...
          {author && <p className="font-sans text-[clamp(6px,4.2cqi,16px)] font-medium uppercase tracking-[0.18em] opacity-90">{author}</p>}
        </div>
      )}
    </div>
  );
